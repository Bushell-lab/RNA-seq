...
#The script is written for paired end reads. If your reads are single ended you need to use SE_1_adaptor_removal.sh
...
do
cutadapt $fastq_dir/${filename}_R1.fastq.gz $fastq_dir/${filename}_R2.fastq.gz -a $Totals_adaptor -A $Totals_adaptor --nextseq-trim=20 -m 30 --cores=$threadN -o $fastq_dir/${filename}_R1_cutadapt.fastq -p $fastq_dir/${filename}_R2_cutadapt.fastq 1> $log_dir/${filename}_cutadapt_log.txt
done

#run fastqc on cutadapt output
for filename in $Totals_filenames
do
fastqc $fastq_dir/${filename}_R1_cutadapt.fastq --outdir=$fastqc_dir &
fastqc $fastq_dir/${filename}_R2_cutadapt.fastq --outdir=$fastqc_dir &
done
wait
