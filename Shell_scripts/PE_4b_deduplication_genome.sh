...
do
umi_tools dedup -I $BAM_dir/${filename}_genome_sorted.bam -S $BAM_dir/${filename}_genome_deduplicated.bam --output-stats=$log_dir/${filename}_genome_deduplication --paired --unpaired-reads discard 1> $log_dir/${filename}_genome_deduplication_log.txt &
done
wait
