...
#The script is written for paired end reads. If your reads are single ended you need to use SE_align_reads_genome.sh
...
do
#STAR --readFilesIn $fastq_dir/${filename}_R1_UMI_clipped.fastq $fastq_dir/${filename}_R2_UMI_clipped.fastq --runThreadN $threadN --genomeDir $STAR_index --outFilterMultimapNmax 5 --outFilterMismatchNmax 5 --outSAMprimaryFlag AllBestScore --alignEndsType EndToEnd --outSAMtype BAM Unsorted --outSAMunmapped None --sjdbGTFfile $STAR_GTF --outFileNamePrefix $STAR_dir/${filename}
STAR --readFilesIn $fastq_dir/${filename}_R1_cutadapt.fastq $fastq_dir/${filename}_R2_cutadapt.fastq --runThreadN $threadN --genomeDir $STAR_index --outFilterMultimapNmax 5 --outFilterMismatchNmax 5 --outSAMprimaryFlag AllBestScore --alignEndsType EndToEnd --outSAMtype BAM Unsorted --outSAMunmapped None --sjdbGTFfile $STAR_GTF --outFileNamePrefix $STAR_dir/${filename}
done
